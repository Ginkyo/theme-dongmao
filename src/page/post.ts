import { documentFunction, dongmao, type ThemeConfig } from "../main";
...
export default class Post {
  ...
  public autoCalculateSubmenuPosition() {
    const reword = dongmao.getThemeConfig("post", "reward", Array<ThemeConfig>);
  ...
  public registerOriginalPostCopy() {
    ...
    if (
      dongmao.getPageConfig("isOriginal") == "false" ||
      !dongmao.getThemeConfig("post", "post_original_copy", Boolean)?.valueOf()
    ) {
      return;
    }
    contentElement.addEventListener("copy", (event) => {
      ...
      ${dongmao.translate("post.copyright_template_html.author", `# 作者(Author): ${owner} <br>`, { postAuthor: owner })}
      ${dongmao.translate("post.copyright_template_html.url", `# 链接(URL): ${url} <br>`, { postUrl: url })}
      ${dongmao.translate("post.copyright_template_html.url", `# 来源(Source): ${siteName} <br>`, {
        siteName: siteName,
      })}
      <br>
      `;
  ...
  public registerWordCountToast() {
    if (!dongmao.getThemeConfig("post", "post_word_count_toast", Boolean)?.valueOf()) {
      return;
    }
    ...
    if (postWordCount > 0) {
      ...
      if (window.innerWidth > 860) {
        ...
        remind =
          dongmao.getThemeConfig("post", `post_word_count_toast_${type.toLowerCase()}`, String)?.valueOf() ||
          dongmao.translate("post.word_count_toast.remind", defaultRemind[type.toLowerCase()], {
  ...
  public registerEditTimeToast() {
    ...
    if (!dongmao.getThemeConfig("post", "post_edit_time_toast", Boolean)?.valueOf()) {
      return;
    }

    if (!dongmao.getPageConfig("postLastModifyTime")) {
      return;
    }

    const postLastModifyTime = dongmao.getPageConfig("postLastModifyTime");
    ...
    if (window.innerWidth > 860) {
      ...
      remind =
        dongmao.getThemeConfig("post", `post_edit_time_toast_${type.toLowerCase()}`, String)?.valueOf() ||
        dongmao.translate("post.edit_time_toast.remind", defaultRemind[type.toLowerCase()], {
          context: type.toLowerCase(),
        });
    }
    ...
    this.createPostToast(
      contentElement,
      dongmao.translate("post.edit_time_toast.content", `文章内容上次编辑时间于 <b>${sinceLastTime}</b>。${remind}`, {
