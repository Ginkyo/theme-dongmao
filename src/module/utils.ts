...
export class Utils {
  ...
  public registerFixedAPlayer() {
    const aplayerFloat = dongmao.getThemeConfig("additional", "aplayer_float", Boolean)?.valueOf();
    if (!aplayerFloat) {
      return;
    }

    const musicHost = dongmao.getThemeConfig("additional", "aplayer_host", String)?.valueOf();
    const musicServer = dongmao.getThemeConfig("additional", "aplayer_server", String)?.valueOf();
    const musicType = dongmao.getThemeConfig("additional", "aplayer_type", String)?.valueOf();
    const musicId = dongmao.getThemeConfig("additional", "aplayer_id", String)?.valueOf();
      ...
      .then((data) => {
        ...
        import("aplayer").then(async (module) => {
          ...
          const flxedAplayerOptions = {
            ...
            mini: true,
            fixed: true,
            autoplay: dongmao.getThemeConfig("additional", "aplayer_autoplay", Boolean)?.valueOf() || false,
            mutex: true,
            lrcType: 3,
            preload: dongmao.getThemeConfig("additional", "aplayer_preload", String)?.valueOf() || "auto",
            theme: dongmao.getThemeConfig("additional", "aplayer_theme", String)?.valueOf() || "#2980b9",
            loop: "all",
            order: dongmao.getThemeConfig("additional", "aplayer_order", String)?.valueOf() || "list",
            volume: dongmao.getThemeConfig("additional", "aplayer_volume", Number)?.valueOf() || null,
            listFolded: false,
            listMaxHeight: "250px",
  ...
  public registerHeaderClass() {
    ...
    if (dongmao.getPageConfig("_templateId") === "index") {
   ...
   * 考虑到，此功能属于常用功能，因此将其注册到 Dongmao 主题中，而不是采取插件的方式。
  ...
  public registerHighlight() {
    ...
    preElements.forEach((preElement) => {
      ...
      import("highlight.js").then(async (highlight) => {
        ...
        const isCodeLine = dongmao.getThemeConfig("post", "code_line", Boolean)?.valueOf();
        // 控制代码是否增加行号
        if (isCodeLine) {
          // @ts-ignore
  ...
  public registerCopyCode() {
    ...
    codeElements.forEach((codeElement) => {
      ...
      copyElement.setAttribute("title", dongmao.translate("common.copy_code", "复制代码"));
  ...
  public registerToc() {
    ...
    tocContainerElements?.forEach((tocContainerElement) => {
      import("tocbot").then((tocbot) => {
        ...
        const collapseDepth = dongmao.getThemeConfig("post", "toc_depth", Number)?.valueOf();
        if (!tocElement) {
          return;
        }
        tocbot.default.init({
