...
export class Events {
  ...
  public searchModal() {
    ...
    if (!dongmao.getPageConfig("showSearchModal")) {
  ...
  public registerCopyEvent() {
    WindowEventProxy.addEventListener(
      ...
      () => {
        if (dongmao.$toast) {
          dongmao.$toast.create(
            dongmao.translate("common.events.copy", "复制成功！<br>Copied to clipboard successfully!"),
            2000
          );
        }
      },
    ...
    listPaginationLinkElement.addEventListener("click", (event) => {
      ...
      fetch(url, {
        ...
        .then((html) => {
          ...
          // 绑定新的 DOM 元素至 pjax 上
          if (dongmao.$pjax) {
            dongmao.$pjax.refresh(postListElement);
          }
          ...
          if (nextPaginationElement) {
            ...
            paginationElement.innerHTML = `<span>${dongmao.translate("page.theend", "没有更多文章了")}</span>`;
          }
        })
        .catch((error) => {
          console.error(error);
        ...
        .finally(() => {
          ...
          targetElement.textContent = dongmao.translate("page.next", "下一页");
          if (dongmao.$localize) {
            dongmao.$localize(".post");
          }
        });
      return false;
    });
  ...
  public registerThemeItemClickEventAndDefaultTheme() {
    ...
    themeItemElements?.forEach((element) => {
      ...
      element.addEventListener("click", () => {
        this.registerThemeRevert(themeData);
        localStorage.setItem("dongmaoTheme", JSON.stringify(themeData));
        // 隐藏主题开关
  ...
  public registerThemeRevert(themeData?: ThemeItemOptions) {
    if (!themeData) {
      const localThemeData = localStorage.getItem("dongmaoTheme");
      if (!localThemeData) {
        return;
      }
